...
public final class Palette<V> {
    ...
    public void readFromStorage(ByteBuf byteBuf, IntDeserializer<V> deserializer, Palette<V> last) {
        var header = byteBuf.readUnsignedByte();
        // Unlike block storage, persistent biome palettes never carry a reliable nbt/non-nbt
        // flag bit: vanilla Bedrock and PocketMine-MP both always write the header as
        // bitsPerBlock << 1 (bit 0 unset) and always encode entries as raw ints, so bit 0
        // can't be trusted here to distinguish nbt-encoded data. Treat it as int-encoded
        // unconditionally to stay compatible with worlds generated by those implementations.
